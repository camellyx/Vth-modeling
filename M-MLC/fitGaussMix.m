...
%         mixpdf_help(x, mu, sigma, alpha, beta, gauss_div(x, mu, sigma));
    ...
    mylog = @(a) log(a+(a==0));
    objfun = @(v) - sum(sum(binpdf .* mylog(binpdf_est(xgap(v(1)), v)))) + sum((v(3:15)<0)*100);
...
%                    'FunValCheck', 'off'))
    ...
    objfun_ER = @(v) objfun([v_def(1) v(1:18)]);
    [vv, err, exitflag, output] = mysearch(objfun_ER, v_def(2:19));
    v_def(2:19) = vv;
    v = v_def;
